...
"""
        ...
        result_benchmark_md_map['environment'] = data['environment']
...
def gen_readme_md(config, output_dir, readme_md_template_filename, readme_md_filename, input_results, img_dir):
    with open(readme_md_template_filename, 'r') as readme_md_template_file:
        ...
        with open(readme_md_filename, 'w') as readme_md_file:
            readme_md = pystache.render(readme_md_template, input_results).rstrip() + '\n'
...
def main(args):
    ...
    if args['gen-plots']:
        ...
        if 'environment' not in input_results:
            cpu_frequency = psu.cpu_freq()
            mhz_per_cpu = cpu_frequency.current if cpu_frequency else 0
            num_cpus = psu.cpu_count(logical=False) or psu.cpu_count() or 0
            input_results['environment'] = {
                'os': platform.system(),
                'cpu': "{:.2f}GHz @ {:d}Cores".format(mhz_per_cpu / 1000.0, num_cpus),
